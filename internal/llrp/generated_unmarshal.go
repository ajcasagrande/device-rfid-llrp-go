...
// UnmarshalBinary Parameter 1023, ImpinjPeakRSSI.
func (p *ImpinjPeakRSSI) UnmarshalBinary(data []byte) error {
	if err := hasEnoughBytes(ParamCustom, 10, len(data), true); err != nil {
		return err
	}
	p.VendorID = binary.BigEndian.Uint32(data)
	p.Subtype = binary.BigEndian.Uint32(data[4:])
	p.RSSI = int16(binary.BigEndian.Uint16(data[8:]))
	return nil
}
