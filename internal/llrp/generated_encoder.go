...
// EncodeFields for Parameter 1023, ImpinjPeakRSSI.
func (p *ImpinjPeakRSSI) getHeader() paramHeader {
	return paramHeader{
		ParamType: ParamCustom,
		data:      p,
		sz:        14,
...
func (p *ImpinjPeakRSSI) EncodeFields(w io.Writer) error {
	if _, err := w.Write([]byte{
		byte(p.VendorID >> 24), byte(p.VendorID >> 16), byte(p.VendorID >> 8), byte(p.VendorID),
		byte(p.Subtype >> 24), byte(p.Subtype >> 16), byte(p.Subtype >> 8), byte(p.Subtype),
		byte(p.RSSI >> 8), byte(p.RSSI)}); err != nil {
		return errors.Wrap(err, "failed to write fields for ParamCustom")
	}
	return nil
}
